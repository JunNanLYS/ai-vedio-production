...
export const assetsService = {
  ...
  createProject: (name: string, path?: string) => api.post<Project>('/api/projects', { project_name: name, project_path: path }),
  loadProject: (path: string) => api.post<{ message: string; project: Project; loaded_assets: number }>('/api/projects/load', { project_path: path }),
  getCurrentProject: () => api.get<{ project: Project | null }>('/api/projects/current'),
  switchProject: (projectId: number) => api.post<{ message: string; project: Project }>(`/api/projects/switch/${projectId}`, {}),
